-- A correction to the same fact remains a new version of the same question.
-- A full content replacement declares replacesCatalogKey and receives a new
-- question row, so prior repeat state cannot reduce the new question's value.
